import { getTokenFromCookie, airtablePatch } from '../../_lib';
...
export default async function handler(req: any, res: any) {
  ...
  for (const [key, value] of Object.entries(incoming)) {
    if (WRITABLE_FIELDS.has(key)) safeFields[key] = value;
  }
  ...
  try {
    const data = await airtablePatch(token, `/${BASE_ID}/${encodeURIComponent(TABLE)}/${id}`, {
      fields: safeFields,
    });
    return res.status(200).json(data);
  } catch (err) {
    console.error('[opportunities/[id]]', err);
    return res.status(500).json({ error: 'Failed to update Airtable record', detail: String(err) });
  }
}
